...
export const processCards = async (req, puppeteer, browser) => {
    const {words, language, deck = 'English'} = req.body;
...
const getWordLocation = (body) => {
    ...
    dataUrl = dataUrl.concat(`/${language.input}${language.output}/${word}`)

    console.log('fsa',dataUrl)
...
const getAudioFiles = (data) => {
    ...
    const getUrl = (searchTerm) => googleTTS.getAudioUrl(searchTerm, {
        lang: language.input, slow: false, host: 'https://translate.google.com',
    });
    const wordAudio = getUrl(word);
    const phraseAudio = getUrl(phrase);
